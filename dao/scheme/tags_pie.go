...
func (ss Tags) All(fn func(value *Tag) bool) bool {
...
func (ss Tags) Any(fn func(value *Tag) bool) bool {
	for _, value := range ss {
		if fn(value) {
			return true
		}
...
func (ss Tags) Append(elements ...*Tag) Tags {
	// Copy ss, to make sure no memory is overlapping between input and
	// output. See issue #97.
	result := append(Tags{}, ss...)
...
func (ss Tags) Bottom(n int) (top Tags) {
...
func (ss Tags) Contains(lookingFor *Tag) bool {
	for _, s := range ss {
		if lookingFor == s {
			return true
		}
...
func (ss Tags) Diff(against Tags) (added, removed Tags) {
	...
	diffOneWay := func(ss1, ss2raw Tags) (result Tags) {
		ss2 := make(Tags, len(ss2raw))
		copy(ss2, ss2raw)
...
// if n < 0 it'll return empty slice.
func (ss Tags) DropTop(n int) (drop Tags) {
	if n < 0 || n >= len(ss) {
		return
	}
...
//   })
//
func (ss Tags) Each(fn func(*Tag)) Tags {
	for _, s := range ss {
		fn(s)
	}
...
func (ss Tags) Equals(rhs Tags) bool {
	if len(ss) != len(rhs) {
		return false
	}
...
//
// It is acceptable to provide zero arguments.
func (ss Tags) Extend(slices ...Tags) (ss2 Tags) {
	ss2 = ss
...
func (ss Tags) Filter(condition func(*Tag) bool) (ss2 Tags) {
	for _, s := range ss {
		if condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss Tags) FilterNot(condition func(*Tag) bool) (ss2 Tags) {
	for _, s := range ss {
		if !condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss Tags) FindFirstUsing(fn func(value *Tag) bool) int {
...
func (ss Tags) First() *Tag {
	return ss.FirstOr(nil)
}

// FirstOr returns the first element or a default value if there are no
// elements.
func (ss Tags) FirstOr(defaultValue *Tag) *Tag {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss Tags) Float64s() pie.Float64s {
	l := len(ss)
...
func (ss Tags) Insert(index int, values ...*Tag) Tags {
	if index >= ss.Len() {
		return Tags.Extend(ss, Tags(values))
	}

	return Tags.Extend(ss[:index], Tags(values), ss[index:])
}

// Ints transforms each element to an integer.
func (ss Tags) Ints() pie.Ints {
	l := len(ss)
...
func (ss Tags) Join(glue string) (s string) {
	var slice interface{} = []*Tag(ss)
...
func (ss Tags) JSONBytesIndent(prefix, indent string) []byte {
	if ss == nil {
		return []byte("[]")
	}
...
// One important thing to note is that it will treat a nil slice as an empty
// slice to ensure that the JSON value return is always an array.
func (ss Tags) JSONString() string {
	if ss == nil {
		return "[]"
	}
...
// slice to ensure that the JSON value return is always an array. See
// json.MarshalIndent for details.
func (ss Tags) JSONStringIndent(prefix, indent string) string {
	if ss == nil {
		return "[]"
	}
...
func (ss Tags) Last() *Tag {
	return ss.LastOr(nil)
}

// LastOr returns the last element or a default value if there are no elements.
func (ss Tags) LastOr(defaultValue *Tag) *Tag {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss Tags) Map(fn func(*Tag) *Tag) (ss2 Tags) {
	if ss == nil {
		return nil
	}
...
func (ss Tags) Mode() Tags {
	if len(ss) == 0 {
		return nil
	}
	values := make(map[*Tag]int)
	for _, s := range ss {
		values[s]++
	}
...
func (ss Tags) Random(source rand.Source) *Tag {
	n := len(ss)
...
func (ss Tags) Send(ctx context.Context, ch chan<- *Tag) Tags {
	for i, s := range ss {
		select {
		case <-ctx.Done():
			return ss[:i]
...
func (ss Tags) SequenceUsing(creator func(int) *Tag, params ...int) Tags {
	var seq = func(min, max, step int) (seq Tags) {
		lenght := int(util.Round(float64(max-min) / float64(step)))
		if lenght < 1 {
			return
		}

		seq = make(Tags, lenght)
...
func (ss Tags) Shift() (*Tag, Tags) {
	return ss.First(), ss.DropTop(1)
}

// Shuffle returns shuffled slice by your rand.Source
func (ss Tags) Shuffle(source rand.Source) Tags {
	n := len(ss)
...
func (ss Tags) SortStableUsing(less func(a, b *Tag) bool) Tags {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
// slice returned will be reallocated as to not modify the input slice.
func (ss Tags) SortUsing(less func(a, b *Tag) bool) Tags {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
//   fmt.Sprintf("%v")
//
func (ss Tags) Strings() pie.Strings {
	l := len(ss)
...
func (ss Tags) SubSlice(start int, end int) (subSlice Tags) {
	if start < 0 || end < 0 {
		return
	}
	...
	if start < length {
		if end <= length {
			...
			zeroArray := make([]*Tag, end-length)
			subSlice = ss[start:length].Append(zeroArray[:]...)
		}
	} else {
		zeroArray := make([]*Tag, end-start)
		subSlice = zeroArray[:]
	}
...
// if the slice has less elements then n that'll return all elements
// if n < 0 it'll return empty slice.
func (ss Tags) Top(n int) (top Tags) {
...
func (ss Tags) StringsUsing(transform func(*Tag) string) pie.Strings {
	l := len(ss)
...
func (ss Tags) Unshift(elements ...*Tag) (unshift Tags) {
	unshift = append(Tags{}, elements...)
