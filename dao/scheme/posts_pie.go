...
func (ss Posts) All(fn func(value *Post) bool) bool {
...
func (ss Posts) Any(fn func(value *Post) bool) bool {
	for _, value := range ss {
		if fn(value) {
			return true
		}
...
func (ss Posts) Append(elements ...*Post) Posts {
	// Copy ss, to make sure no memory is overlapping between input and
	// output. See issue #97.
	result := append(Posts{}, ss...)
...
func (ss Posts) Bottom(n int) (top Posts) {
...
func (ss Posts) Contains(lookingFor *Post) bool {
	for _, s := range ss {
		if lookingFor == s {
			return true
		}
...
func (ss Posts) Diff(against Posts) (added, removed Posts) {
	...
	diffOneWay := func(ss1, ss2raw Posts) (result Posts) {
		ss2 := make(Posts, len(ss2raw))
		copy(ss2, ss2raw)
...
// if n < 0 it'll return empty slice.
func (ss Posts) DropTop(n int) (drop Posts) {
	if n < 0 || n >= len(ss) {
		return
	}
...
//   })
//
func (ss Posts) Each(fn func(*Post)) Posts {
	for _, s := range ss {
		fn(s)
	}
...
func (ss Posts) Equals(rhs Posts) bool {
	if len(ss) != len(rhs) {
		return false
	}
...
//
// It is acceptable to provide zero arguments.
func (ss Posts) Extend(slices ...Posts) (ss2 Posts) {
	ss2 = ss
...
func (ss Posts) Filter(condition func(*Post) bool) (ss2 Posts) {
	for _, s := range ss {
		if condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss Posts) FilterNot(condition func(*Post) bool) (ss2 Posts) {
	for _, s := range ss {
		if !condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss Posts) FindFirstUsing(fn func(value *Post) bool) int {
...
func (ss Posts) First() *Post {
	return ss.FirstOr(nil)
}

// FirstOr returns the first element or a default value if there are no
// elements.
func (ss Posts) FirstOr(defaultValue *Post) *Post {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss Posts) Float64s() pie.Float64s {
	l := len(ss)
...
func (ss Posts) Insert(index int, values ...*Post) Posts {
	if index >= ss.Len() {
		return Posts.Extend(ss, Posts(values))
	}

	return Posts.Extend(ss[:index], Posts(values), ss[index:])
}

// Ints transforms each element to an integer.
func (ss Posts) Ints() pie.Ints {
	l := len(ss)
...
func (ss Posts) Join(glue string) (s string) {
	var slice interface{} = []*Post(ss)
...
func (ss Posts) JSONBytesIndent(prefix, indent string) []byte {
	if ss == nil {
		return []byte("[]")
	}
...
// One important thing to note is that it will treat a nil slice as an empty
// slice to ensure that the JSON value return is always an array.
func (ss Posts) JSONString() string {
	if ss == nil {
		return "[]"
	}
...
// slice to ensure that the JSON value return is always an array. See
// json.MarshalIndent for details.
func (ss Posts) JSONStringIndent(prefix, indent string) string {
	if ss == nil {
		return "[]"
	}
...
func (ss Posts) Last() *Post {
	return ss.LastOr(nil)
}

// LastOr returns the last element or a default value if there are no elements.
func (ss Posts) LastOr(defaultValue *Post) *Post {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss Posts) Map(fn func(*Post) *Post) (ss2 Posts) {
	if ss == nil {
		return nil
	}

	ss2 = make([]*Post, len(ss))
	for i, s := range ss {
		ss2[i] = fn(s)
	}
...
func (ss Posts) Mode() Posts {
	if len(ss) == 0 {
		return nil
	}
	values := make(map[*Post]int)
	for _, s := range ss {
		values[s]++
	}
...
func (ss Posts) Random(source rand.Source) *Post {
	n := len(ss)
...
func (ss Posts) Send(ctx context.Context, ch chan<- *Post) Posts {
	for i, s := range ss {
		select {
		case <-ctx.Done():
			return ss[:i]
...
func (ss Posts) SequenceUsing(creator func(int) *Post, params ...int) Posts {
	var seq = func(min, max, step int) (seq Posts) {
		lenght := int(util.Round(float64(max-min) / float64(step)))
		if lenght < 1 {
			return
		}

		seq = make(Posts, lenght)
...
func (ss Posts) Shift() (*Post, Posts) {
	return ss.First(), ss.DropTop(1)
}

// Shuffle returns shuffled slice by your rand.Source
func (ss Posts) Shuffle(source rand.Source) Posts {
	n := len(ss)
...
func (ss Posts) SortStableUsing(less func(a, b *Post) bool) Posts {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
// slice returned will be reallocated as to not modify the input slice.
func (ss Posts) SortUsing(less func(a, b *Post) bool) Posts {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
//   fmt.Sprintf("%v")
//
func (ss Posts) Strings() pie.Strings {
	l := len(ss)
...
func (ss Posts) SubSlice(start int, end int) (subSlice Posts) {
	if start < 0 || end < 0 {
		return
	}
	...
	if start < length {
		if end <= length {
			...
			zeroArray := make([]*Post, end-length)
			subSlice = ss[start:length].Append(zeroArray[:]...)
		}
	} else {
		zeroArray := make([]*Post, end-start)
		subSlice = zeroArray[:]
	}
...
// if the slice has less elements then n that'll return all elements
// if n < 0 it'll return empty slice.
func (ss Posts) Top(n int) (top Posts) {
...
func (ss Posts) StringsUsing(transform func(*Post) string) pie.Strings {
	l := len(ss)
...
func (ss Posts) Unshift(elements ...*Post) (unshift Posts) {
	unshift = append(Posts{}, elements...)
